...
/// Indicates how to treat the edges of a directed graph.
pub enum EdgeDirection {
    /// Keep the direction as in the graph G.
    Forward,
    ...
    N: Clone + Eq + Hash + PartialEq,
{
        ...
        NI: IntoIterator<Item = N>,
    {
        ...
        let extended_edges = self
            .graph
            .edges()
            .map(|(parent, child)| match direction {
                EdgeDirection::Forward => {
                    (NodeTag::Real(parent.clone()), NodeTag::Real(child.clone()))
                }
                EdgeDirection::Reverse => {
                    (NodeTag::Real(child.clone()), NodeTag::Real(parent.clone()))
                }
            })
            .chain(std::iter::once(match direction {
                EdgeDirection::Forward => {
                    (NodeTag::Virtual, NodeTag::Real(self.start_node.clone()))
                }
                EdgeDirection::Reverse => {
                    (NodeTag::Real(self.start_node.clone()), NodeTag::Virtual)
                }
            }));
    ...
    V: Hash + Eq + Clone,
{
    ...
    pub fn find_dominator_value(
        &self,
        final_nodes: &[N],
        direction: EdgeDirection,
    ) -> Result<Option<V>, FlowGraphError> {
        ...
        Ok(self
            .value_flow_graph
            .find_closest_common_dominator(final_values, direction))
    }
}
...
mod tests {
    ...
    fn test_flow_graph_find_closest_common_dominator() {
        ...
        let flow_graph = FlowGraph::new(simple_graph, "A").unwrap();
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["C", "D"], EdgeDirection::Forward),
            Some("C")
        );
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["B", "D"], EdgeDirection::Forward),
            Some("B")
        );
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["A", "D"], EdgeDirection::Forward),
            Some("A")
        );
        ...
        let flow_graph = FlowGraph::new(simple_graph, "A").unwrap();
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["B", "C"], EdgeDirection::Forward),
            Some("A")
        );
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["B", "D"], EdgeDirection::Forward),
        ...
        assert_eq!(
            flow_graph.find_closest_common_dominator(vec!["D"], EdgeDirection::Forward),
            Some("D")
        );
    ...
    fn test_value_flow_graph_find_dominator_value() {
        ...
        //     \--------|
        assert_eq!(
            value_flow_graph.find_dominator_value(&["D", "E"], EdgeDirection::Forward),
            Ok(Some(3))
        );
        assert_eq!(
            value_flow_graph.find_dominator_value(&["C", "D"], EdgeDirection::Forward),
            Ok(Some(1))
        );
        assert_eq!(
            value_flow_graph.find_dominator_value(&["B", "C"], EdgeDirection::Forward),
            Ok(Some(1))
        );
        ...
        let value_flow_graph = ValueFlowGraph::new(&flow_graph, &value_fn).unwrap();
        assert_eq!(
            value_flow_graph.find_dominator_value(&["B", "D"], EdgeDirection::Forward),
            Ok(None)
        );
    }
}
